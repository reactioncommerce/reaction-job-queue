...
export function createJobCollectionClass({ Meteor, Job, JobCollectionBaseClass }) {
  return class JobCollection extends JobCollectionBaseClass {
    constructor(root = "queue", options = {}) {
      ...
      this.setInterval = Meteor.setInterval;
      this.methods = Meteor.methods;
      this.clearInterval = Meteor.clearInterval;
      this.Error = Meteor.Error;
      ...
      return function (...params) {
        ...
        try {
          if (!this.connection || !!permitted(this.userId, params)) {
            ...
            err = new this.Error(403, "Method not authorized", "Authenticated user is not permitted to invoke this method.");
            throw err;
          }
        } catch (error) {
          err = error;
    ...
    deny(denyOptions) {
      return (() => {
        ...
        for (const type in denyOptions) {
          if ({}.hasOwnProperty.call(denyOptions, type)) {
    ...
    promote(milliseconds = 15 * 1000) {
      if ((typeof milliseconds === "number") && (milliseconds > 0)) {
        ...
        this.promoteJobs();
        this.interval = this.setInterval(this.promoteJobs.bind(this), milliseconds);
